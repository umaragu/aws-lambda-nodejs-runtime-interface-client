...
export async function run(appRoot: string, handler: string, initFunction: any): Promise<void> {
  ...
  const handlerFunc = await UserFunction.load(appRoot, handler, initFunction) as HandlerFunction;
